...
public class EventServiceTest {
    ...
    public void testUpdateEvent() {
        ...
        // Crie um objeto EventUpdateDTO com os novos dados do evento
        EventUpdateDTO eventUpdateDTO = new EventUpdateDTO();
        eventUpdateDTO.setName("Meu evento atualizado");
        eventUpdateDTO.setDescription("Um evento atualizado incrível");
        eventUpdateDTO.setStartDate(LocalDate.of(2023, 11, 1));
        eventUpdateDTO.setEndDate(LocalDate.of(2023, 11, 3));
        eventUpdateDTO.setStartTime(LocalTime.of(9, 0));
        eventUpdateDTO.setEndTime(LocalTime.of(18, 0));
        eventUpdateDTO.setEmailContact("contato@meueventoatualizado.com");
        eventUpdateDTO.setPriceTickets(75.0);

        event.setId(eventId);

        // Mock o comportamento do eventRepository.getOne(id) para retornar o existingEvent
        when(eventRepository.getOne(eventId)).thenReturn(event);

        // Chame o método updateEvent
        when(eventRepository.save(event)).thenReturn(event);
        EventDTO updatedEventDTO = eventService.updateEvent(eventId, eventUpdateDTO);

        // Verifique se o evento foi atualizado corretamente
        assertEquals(eventUpdateDTO.getName(), updatedEventDTO.getName());
        assertEquals(eventUpdateDTO.getDescription(), updatedEventDTO.getDescription());
        assertEquals(eventUpdateDTO.getStartDate(), updatedEventDTO.getStartDate());
        assertEquals(eventUpdateDTO.getEndDate(), updatedEventDTO.getEndDate());
    }
